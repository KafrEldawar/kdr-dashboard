-- ============================================================
-- 043: Persistent verified alternate phone on profiles
--
-- Previously the checkout screen accepted a free-form alternate
-- phone per order (stored on orders.alternate_phone by 037) but
-- the value was never verified and never saved. This migration
-- promotes the alternate phone to a first-class profile field
-- that the customer can add, verify via OTP, reuse across orders,
-- and remove from a bottom sheet without leaving checkout.
--
-- Verification reuses the existing send-otp / verify-otp edge
-- functions with purpose='attach' (the column already accepts it
-- via 033's CHECK). The edge function writes the columns directly
-- after the code is consumed.
-- ============================================================

alter table public.profiles
  add column if not exists alternate_phone             text,
  add column if not exists alternate_phone_verified_at timestamptz;

-- Alt cannot equal the primary phone.
alter table public.profiles
  drop constraint if exists profiles_alternate_phone_distinct;
alter table public.profiles
  add  constraint profiles_alternate_phone_distinct
       check (alternate_phone is null or alternate_phone <> phone);

-- One owner per phone — mirrors the implicit uniqueness of `phone`
-- (auth.users + idx_profiles_phone) so two accounts can't claim the
-- same alternate.
create unique index if not exists idx_profiles_alternate_phone
  on public.profiles (alternate_phone)
  where alternate_phone is not null;


-- ── rpc_remove_alternate_phone ────────────────────────────────
-- Clears both columns. Idempotent: returns the (refreshed) profile
-- whether or not anything was actually deleted.
create or replace function public.rpc_remove_alternate_phone()
...
begin
  ...
  update profiles set
    ...
    alternate_phone_verified_at = null,
    updated_at                  = now()
  where id = v_uid;
...
grant execute on function public.rpc_remove_alternate_phone() to authenticated;


-- ============================================================
-- rpc_checkout v5 — guard the alternate phone against forgery.
--
-- The signature is unchanged (the v4 from 037 already accepts
-- p_alternate_phone), but the body now requires the value to
-- match the caller's saved + verified alternate. Free-form
-- numbers are rejected — the mobile app must take the user
-- through the bottom-sheet OTP flow first.
--
-- We replace the function in place; the grant from 037 carries
-- over because the signature didn't change.
-- ============================================================
...
begin
  ...
  -- The alt is opt-in. When the caller sends one, it MUST be the
  -- saved + verified value on the profile. Free-form input is no
  -- longer accepted — the mobile app now forces verification via
  -- a bottom sheet before the value can be attached to an order.
  if v_alt_phone is not null then
    select alternate_phone, alternate_phone_verified_at
    into   v_saved_alt, v_saved_alt_verif
    from   profiles where id = v_uid;

    if v_saved_alt is null or v_saved_alt_verif is null then
      return jsonb_build_object('error', 'alternate_phone_not_verified');
    end if;
    if v_saved_alt <> v_alt_phone then
      return jsonb_build_object('error', 'alternate_phone_mismatch');
    end if;
  end if;
